...
import * as postModel from "./cms/models/post.js";
import * as projectModel from "./cms/models/project.js";
const { Pool } = pg;
...
async function initDatabase() {
	...
	try {
		...
		await pool.query(`
			...
			ALTER TABLE cms_projects ADD COLUMN IF NOT EXISTS thumbnail TEXT;
			ALTER TABLE cms_projects ADD COLUMN IF NOT EXISTS star_situation TEXT;
...
/** Absolute site origin for canonical URLs (sitemap, emails). */
function siteOrigin(req) {
	const env = (process.env.APP_BASE_URL || process.env.SITE_URL || "").trim().replace(/\/$/, "");
	if (env) return env;
	const host = (req.headers.host || "localhost:3000").trim();
	const xfProto = (req.headers["x-forwarded-proto"] || "").split(",")[0].trim();
	const proto = xfProto || (host.startsWith("localhost") || host.startsWith("127.0.0.1") ? "http" : "https");
	return `${proto}://${host}`;
}
...
async function sendSitemapBody(req, res, method) {
	...
	if (dbPool) {
		const [postsResult, projectsResult] = await Promise.all([postModel.findAll(false), projectModel.findAll(false)]);
		if (projectsResult.projects) {
			for (const p of projectsResult.projects) {
				if (!p.slug) continue;
				urls.push({
					loc: `${base}/project?slug=${encodeURIComponent(p.slug)}`,
